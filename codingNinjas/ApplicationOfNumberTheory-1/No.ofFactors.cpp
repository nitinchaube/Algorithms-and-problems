...
void makeSieve(int i,int n,int* isPrimes){
    if(isPrimes[i]==0){
        for(int j=1;j*i<=n;j++){
            isPrimes[j*i]+=1;
        }
...
int main(){
    ...
    for(int i=2;i<=1000001;i++){        //making the sieve for the nos dividing by prime nos.
        makeSieve(i,1000001,dp);
    }

    // table for all the nos and their prime divisor counts
