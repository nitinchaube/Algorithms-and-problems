...
bool compare(labour A,labour B){
    if(A.time==B.time)
    {
        if(A.speed==B.speed)
        {
...
double fractionalK(labour *arr,ll N, ll D){
    sort(arr,arr+N,compare);

    ll current_cost=arr[0].cost;
    ll current_area_covered=0;
    ll current_worker_index=0;
    for(int i=1;i<N && current_area_covered<D;i++){
        current_area_covered+=(arr[i].time-arr[i-1].time)*arr[current_worker_index].speed;
        if(current_area_covered>=D){
            return current_cost;
        }
        if(arr[current_worker_index].speed < arr[i].speed){
            current_worker_index=i;
            current_cost+=arr[current_worker_index].cost;
        }
    }
    return current_cost;
...
int main()
{
    ll N,D;
    cin>>N>>D;
    labour *arr=new labour[N];
    for(ll i=0;i<N;i++){
        cin>>arr[i].time>>arr[i].cost>>arr[i].speed;
    }

    cout<<fractionalK(arr,N,D)<<endl;
